const Person = function(name, dateOfBirth, initialAmount) {
    this.name = name;
...
Person.prototype._getAge = function() {
    ...
    return (monthNow === month && dayNow < day || monthNow < month) ? yearNow - year - 1 : yearNow - year;
};
...
Person.prototype.addTransactionToHistory = function(transaction) {
    this.history.push(transaction);
};

Person.prototype.addMoney = function(amount, description) {
    this.amount += amount;
    this.addTransactionToHistory({description: description, amount: amount});
};

Person.prototype.withdrawMoney = function(amount, description) {
